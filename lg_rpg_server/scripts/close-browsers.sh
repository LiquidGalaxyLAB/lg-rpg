...
for ((i=1; i<=TOTAL_SCREENS; i++)); do
  if [ "$i" -ne 1 ]; then
    ...
    sshpass -p 'lg' ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 "lg@${SLAVE}" "pkill -TERM -f 'chromium-browser|chromium|chrome'; sleep 1; pgrep -af 'chromium|chrome' || echo stopped" 2>/dev/null || echo "Could not SSH to ${SLAVE}"
  fi
done
